...
async def _embed_results(embedding_provider, results: List[SearchResult]):
    ...
    texts_to_embed = [
        _truncate(f"{r.title or ''} {r.excerpt or ''}")
        for r in results
        if not r.vectors
    ]
...
async def search(
    ...
    logger.info(
        f"Search request: q='{q}', lang={lang.value}, use_cse={use_cse}, use_reranking={use_reranking}"
    )
    ...
    if not typesense_client:
        raise HTTPException(
            ...
            detail="Typesense is not available.",
        )
            ...
            for idx, hit in enumerate(hits_list):
                ...
                search_results.append(
                    SearchResult(
                        ...
                        site=doc.get("site"),
                        images=[
                            ImageResult(**img) if isinstance(img, dict) else img
                        ...
                        vectors=doc.get("embedding_vec")
                        if (RERANKING_ENABLED and use_reranking)
                        else None,
                    )
                )
    ...
    async def search_cse() -> Tuple[List[SearchResult], bool, float]:
        ...
        s = time.time()
        try:
            res, hit = await cse_client.search(
                query=q, lang=lang.value, num_results=min(limit, 10)
            )
    ...
    async def search_wiki() -> Tuple[List[SearchResult], float]:
        ...
        wiki_results_list = await asyncio.gather(
            *[search_single_wiki(client) for client in wiki_clients]
        )
    ...
    query_embedding_task = (
        asyncio.to_thread(embedding_provider.encode, [q])
        if RERANKING_ENABLED and embedding_provider and use_reranking
        else None
    )

    (
        (ts_res, ts_time),
        (cse_res, cache_hit, cse_time),
        (wiki_res, wiki_time),
        query_emb_list,
        ...
        search_cse(),
        search_wiki(),
        query_embedding_task or asyncio.sleep(0, result=[None]),
    )

    query_embedding = (
        np.array(query_emb_list[0]) if query_emb_list and query_emb_list[0] else None
    )
    ...
    logger.info(
        f"Results after safety filter: Typesense={len(ts_res)}, CSE={len(cse_res)}, Wiki={len(wiki_res)}"
    )
    ...
    merged_results = deduped_wiki_res + merger.merge(
        typesense_results=ts_res, cse_results=cse_res, limit=limit
    )
    ...
    if use_reranking and RERANKING_ENABLED and reranker and query_embedding is not None:
        ...
        logger.info(
            f"Reranking applied in {reranking_time_ms:.2f}ms, results after rerank: {len(merged_results)}"
        )
    else:
        logger.info(
            f"Reranking skipped (enabled={use_reranking}, configured={RERANKING_ENABLED}, reranker={reranker is not None}, embedding={query_embedding is not None})"
        )
    ...
    logger.info(
        f"Search completed in {total_time_ms:.2f}ms - Breakdown: Typesense={ts_time:.1f}ms, CSE={cse_time:.1f}ms, Wiki={wiki_time:.1f}ms, Rerank={reranking_time_ms:.1f}ms"
        if reranking_time_ms
        else f"Search completed in {total_time_ms:.2f}ms - Breakdown: Typesense={ts_time:.1f}ms, CSE={cse_time:.1f}ms, Wiki={wiki_time:.1f}ms"
    )
    ...
    if state.stats_db:
        state.stats_db.log_search(
            q, lang.value, limit, use_cse, use_hybrid, use_reranking, stats.model_dump()
        )
...
async def submit_feedback(
    request: Request, feedback: FeedbackRequest
) -> FeedbackResponse:
    state: AppState = request.app.state
    if state.stats_db:
