...
def migrate_json_to_db():
    ...
    try:
        ...
        print(
            "❌ Impossible de déterminer les chemins des fichiers. Exécutez ce script directement."
        )
        return

    if not os.path.exists(cache_file):
        print(
            f"✅ Le fichier cache JSON '{os.path.basename(cache_file)}' n'existe pas. Aucune migration nécessaire."
        )
        return
    ...
    try:
        ...
        print(
            f"✓ Configuration de {len(sites_config)} sites chargée depuis '{os.path.basename(sites_yml)}'."
        )
    except FileNotFoundError:
        print(
            f"⚠️  Fichier '{os.path.basename(sites_yml)}' introuvable. Le 'site_name' sera déduit du domaine."
        )
        ...
        for site_name, session_data in old_cache["_meta"]["crawls"].items():
            sessions_to_migrate.append(
                (
                    ...
                    json.dumps(session_data.get("resume_from"))
                    if session_data.get("resume_from")
                    else None,
                )
            )
