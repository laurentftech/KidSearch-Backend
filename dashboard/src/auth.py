...
def _verify_jwt_token(token: str) -> Optional[Dict]:
    ...
    try:
        payload = jwt.decode(
            token, api_config["jwt_secret"], algorithms=[api_config["jwt_algorithm"]]
        )
        return payload
...
def check_authentication():
    ...
    if auth_config.has_provider(AuthProvider.PROXY):
        ...
        auth_email = headers.get("x-token-user-email") or headers.get(
            "X-Token-User-Email"
        )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    get_local_storage().setItem(
                        "auth_token", jwt_token, key="store_proxy_token"
                    )
                    ...
                    auth_logger.error(
                        f"Failed to get JWT from API: {response.status_code}"
                    )
            except Exception as e:
    ...
    if auth_token:
        ...
        if payload:
            ...
            auth_logger.warning(
                "Invalid or expired JWT found in localStorage. Clearing."
            )
            get_local_storage().deleteItem("auth_token", key="clear_invalid_token")
            get_local_storage().deleteItem(
                "auth_session_id", key="clear_invalid_session"
            )
    ...
    if session_id:
        ...
        if session_data:
            ...
            auth_logger.info(
                f"Authenticated via persistent session for {session_data.get('email')}"
            )
            return st.session_state.user_info
        else:
            auth_logger.warning(
                f"Persistent session expired for session_id: {session_id}. Clearing."
            )
            get_local_storage().deleteItem(
                "auth_session_id", key="clear_expired_session"
            )
    ...
    auth_logger.warning(
        f"No authentication found. Configured providers: {auth_config.providers}"
    )
    ...
    if not form_providers:
        auth_logger.error(
            f"No form providers available. All providers: {auth_config.providers}, Proxy configured: {auth_config.has_provider(AuthProvider.PROXY)}"
        )
        st.error(t("no_auth_configured"))
        st.info(t("auth_config_help"))
        st.warning(
            "⚠️ Si vous utilisez l'authentification par proxy (Caddy), cette page ne devrait jamais s'afficher. Vérifiez votre configuration Caddy."
        )
        st.code(
            f"AUTH_PROVIDERS={os.getenv('AUTH_PROVIDERS')}\nAUTH_PROXY_ENABLED={os.getenv('AUTH_PROXY_ENABLED')}",
            language="bash",
        )
        st.stop()
...
def _simple_auth(t):
    ...
    with st.form("password_form"):
        ...
        password = st.text_input(
            t("password"), type="password", key="simple_password_input"
        )
...
def _sso_auth(provider, t):
    ...
    st.info(
        f"🔄 L'authentification {provider.upper()} n'est pas encore implémentée dans cette version."
    )
    st.markdown(
        "Pour utiliser l'authentification SSO, configurez l'authentification par proxy avec Caddy + authcrunch."
    )
