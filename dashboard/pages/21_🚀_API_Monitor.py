...
API_DISPLAY_HOST = os.getenv(
    "API_DISPLAY_HOST", API_HOST
)  # URL shown to users (may be behind reverse proxy)
...
API_DISPLAY_URL = (
    f"http://{API_DISPLAY_HOST}"
    + (f":{API_PORT}" if API_DISPLAY_HOST == API_HOST else "")
    + "/api"
)
...
def get_api_data():
    ...
    try:
        ...
        health_response = requests.get(
            f"{API_BASE_URL}/health", timeout=5, headers=headers
        )
        stats_response = requests.get(
            f"{API_BASE_URL}/stats", timeout=5, headers=headers
        )
        ...
        stats_data = (
            stats_response.json() if stats_response.status_code == 200 else None
        )
...
if stats_data:
    ...
    with col4:
        ...
        st.metric("Taux d'erreur", f"{error_rate:.1f}%", delta_color="inverse")
...
if st.session_state.confirm_reset:
    st.warning(
        "Êtes-vous sûr de vouloir supprimer toutes les statistiques de recherche ? Cette action est irréversible."
    )
    ...
    with st.spinner("Recherche en cours..."):
        try:
            ...
            if response.status_code == 200:
                ...
                st.success(
                    f"✅ {len(data['results'])} résultats trouvés en {data['stats']['processing_time_ms']:.0f}ms"
                )
                ...
                if data["results"]:
                    ...
                    for idx, result in enumerate(data["results"][:5], 1):
                        with st.expander(
                            f"{idx}. {result['title']} ({result['source']})"
                        ):
...
with col1:
    refresh_rate = st.slider(
        ...
        10,
        120,
        30,  # Default to 30s for better performance
