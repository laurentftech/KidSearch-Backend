...
with st.form(key="search_form"):
    ...
    with col1:
        query = st.text_input(
            t("search.search_box_label"),
            "",
            placeholder=t("search.search_box_placeholder"),
        )
    with col2:
        use_hybrid = st.checkbox(
            t("search.use_hybrid_search"),
            value=True,
            help=t("search.hybrid_search_help"),
        )
    ...
    submitted = st.form_submit_button(
        label=t("search.search_button_label"), type="primary"
    )
    ...
    with st.spinner(t("search.spinner_searching")):
        ...
        try:
            ...
            if query_lang in ["fr", "en"]:
                filters.append(
                    f"lang:={query_lang}"
                )  # Typesense syntax for exact match
        ...
        if filters:
            search_params["filter_by"] = " && ".join(
                filters
            )  # Typesense uses && for AND
        ...
        if use_hybrid:
            ...
            st.warning(
                "⚠️ Hybrid search requires vector generation which is not yet implemented in this dashboard view. Falling back to keyword search."
            )
        ...
        try:
            ...
            st.success(
                t("search.results_summary").format(
                    count=len(hits),
            ...
            if not hits:
                ...
                for hit in hits:
                    ...
                    with col2:
                        title = highlight.get("title", {}).get(
                            "value", document.get("title", t("search.no_title"))
                        )
                        excerpt = highlight.get("excerpt", {}).get(
                            "snippet", document.get("excerpt", "")
                        )
                        ...
                        st.markdown(
                            f"<small>{t('search.site')}: **{site}**</small>",
                            unsafe_allow_html=True,
                        )
                        if timestamp:
                            date = datetime.fromtimestamp(timestamp)
                            st.markdown(
                                f"<small>{t('search.published_on')}: {date.strftime('%d/%m/%Y')}</small>",
                                unsafe_allow_html=True,
                            )
            ...
            st.error(
                f"{t('search.error_during_search')}: An unexpected error occurred: {e}"
            )
