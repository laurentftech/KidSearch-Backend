...
if client:
    try:
        ...
        try:
            ...
            embeddings_configured = embedding_provider in [
                "gemini",
                "huggingface",
            ...
            if not has_vectors and embeddings_configured:
                st.warning(
                    "🚨 La configuration des embeddings est manquante dans le schéma de la collection."
                )
                st.info(
                    "Pour activer la recherche vectorielle hybride, vous devez recréer la collection avec les embeddings activés."
                )

            with st.expander("⚠️ Recréer la collection (DANGER)"):
                st.warning(
                    "⚠️ **ATTENTION**: Cette action va supprimer tous les documents existants!"
                )
                st.info(
                    "💡 La collection sera recréée avec les embeddings si EMBEDDING_PROVIDER est configuré dans .env"
                )

                st.caption(
                    "Les caches et statistiques suivants seront aussi supprimés:"
                )
                st.code("""
...
• data/status.json
                ...
                confirm_text = st.text_input(
                    "Tapez 'RECREER' pour confirmer:", key="confirm_recreate"
                )

                col1, col2 = st.columns([1, 3])
                with col1:
                    if st.button(
                        "🗑️ Recréer la collection",
                        type="primary",
                        disabled=(confirm_text != "RECREER"),
                    ):
                        try:
                            ...
                            for cache_file in cache_files:
                                try:
                                    ...
                                    st.warning(
                                        f"⚠️ Impossible de supprimer {cache_file}: {e}"
                                    )

                            if files_deleted:
                                st.info(
                                    f"✓ {len(files_deleted)} fichier(s) de cache supprimé(s)"
                                )
                            ...
                            embedding_dimensions = (
                                384  # Default for paraphrase-multilingual-MiniLM-L12-v2
                            )
                            ...
                            st.success(
                                f"✅ Collection '{INDEX_NAME}' recréée avec succès!"
                            )
                            if embeddings_configured:
                                st.success("✅ Les embeddings sont maintenant activés!")
                                st.info(
                                    "💡 Relancez le crawler pour indexer les pages avec les embeddings."
                                )

                            # Show cache cleanup summary
                            if files_deleted:
                                st.success(
                                    f"✅ Caches et statistiques nettoyés ({len(files_deleted)} fichiers)"
                                )
                            ...
                            st.error(
                                f"❌ Erreur lors de la recréation: {recreate_error}"
                            )
            ...
            st.info(
                "The collection will be created automatically when you start the crawler or API server."
            )
        ...
        st.info(
            "Please check that Typesense is running and the connection settings are correct."
        )
else:
