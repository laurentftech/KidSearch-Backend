    ...
    if not documents:
        ...
        for doc in documents:
            ...
            if indexed_at:
                try:
                    ...
                    if freshness_days < 1:
                        freshness_category, freshness_color = (t("tree.indexed_today"), "#22c55e")
                    elif freshness_days < 7:
                        freshness_category, freshness_color = (t("tree.indexed_this_week"), "#84cc16")
                    elif freshness_days < 30:
                        freshness_category, freshness_color = (t("tree.indexed_this_month"), "#eab308")
                    elif freshness_days < 90:
                        freshness_category, freshness_color = (t("tree.indexed_1_3_months"), "#f97316")
                    else:
                        freshness_category, freshness_color = (t("tree.indexed_3_plus_months"), "#ef4444")
                except Exception as e:
            ...
            if last_crawled:
                try:
                    ...
                    if last_crawl_days < 1:
                        ...
                        last_crawl_text = t("tree.crawled_days_ago").format(days=last_crawl_days)
                except Exception:
                    pass
            ...
            if url not in data_map:
                ...
                if last_crawl_timestamp:
                    ...
                    if crawl_age_days < 1:
                        ...
                        crawl_text = t("tree.crawled_days_ago").format(days=crawl_age_days)
                data_map[url] = {
        ...
        if df.empty:
            ...
            if pending_count > 0:
                ...
                col5.metric(t("tree.crawled_less_than_7_days"), len(df[df['last_crawl_days'] < 7]))

            st.markdown("---")
            col1, col2, col3 = st.columns(3)
            if stale_pages := len(df[df['last_crawl_days'] > 30]):
                col1.metric(t("tree.to_recrawl"), stale_pages, help=t("tree.to_recrawl_help"))
            if old_indexed := len(df[df['freshness_days'] > 90]):
                col2.metric(t("tree.old_indexing"), old_indexed, help=t("tree.old_indexing_help"))
            if pending_count > 0:
            ...
            for site_name in df['site'].unique():
                ...
                colors.append(30)
                hover_texts.append(t("tree.site_hover").format(site=site_name, count=len(site_data)))
                ids.append(site_id)
                ...
                for _, row in site_data.iterrows():
                    ...
                    for part in row['path_parts'][:-1]:
                        ...
                        if current_path not in node_map:
                            ...
                            values.append(1)
                            colors.append(30)
                            hover_texts.append(t("tree.folder_hover").format(folder=part))
