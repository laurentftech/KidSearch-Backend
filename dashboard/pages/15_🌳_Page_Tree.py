...
try:
    ...
    with st.spinner(t("tree.loading_sites")):
        try:
            ...
            facet_result = typesense_client.collections[INDEX_NAME].documents.search(
                facet_params
            )
            if (
                facet_result.get("facet_counts")
                and len(facet_result["facet_counts"]) > 0
            ):
    ...
    with col1:
        viz_type = st.selectbox(
            ...
            help=t("tree.viz_help"),
        )
    with col2:
        max_pages = st.slider(
            t("tree.max_pages"), 50, 1000, 200, 50, help=t("tree.max_pages_help")
        )
    with col3:
        filter_site = st.selectbox(
            t("tree.filter_site"), sites_list, help=t("tree.filter_site_help")
        )
    ...
    if not documents:
        ...
        for doc in documents:
            ...
            indexed_at = (
                doc.get("indexed_at")
                or doc.get("last_modified")
                or doc.get("timestamp")
            )
            last_crawled = doc.get("last_crawled_at") or indexed_at

            freshness_days, freshness_category, freshness_color = (
                999,
                t("tree.unknown_date"),
                "#6b7280",
            )
            if indexed_at:
                try:
                    indexed_date = (
                        datetime.fromtimestamp(indexed_at)
                    ...
                    if freshness_days < 1:
                        freshness_category, freshness_color = (
                            t("tree.indexed_today"),
                            "#22c55e",
                        )
                    elif freshness_days < 7:
                        freshness_category, freshness_color = (
                            t("tree.indexed_this_week"),
                            "#84cc16",
                        )
                    elif freshness_days < 30:
                        freshness_category, freshness_color = (
                            t("tree.indexed_this_month"),
                            "#eab308",
                        )
                    elif freshness_days < 90:
                        freshness_category, freshness_color = (
                            t("tree.indexed_1_3_months"),
                            "#f97316",
                        )
                    else:
                        freshness_category, freshness_color = (
                            t("tree.indexed_3_plus_months"),
                            "#ef4444",
                        )
                except Exception as e:
                    freshness_category = t("tree.date_error").format(e=str(e)[:20])

            last_crawl_days, last_crawl_text = (999, t("tree.never_crawled"))
            if last_crawled:
                try:
                    crawled_date = (
                        datetime.fromtimestamp(last_crawled)
                        if isinstance(last_crawled, (int, float))
                        else datetime.fromisoformat(
                            str(last_crawled).replace("Z", "+00:00")
                        )
                    )
                    last_crawl_days = (now - crawled_date.replace(tzinfo=None)).days
                    if last_crawl_days < 1:
                        last_crawl_text = t("tree.crawled_today")
                    else:
                        last_crawl_text = t("tree.crawled_days_ago").format(
                            days=last_crawl_days
                        )
                except Exception:
                    pass
            ...
            data_map[url] = {
                ...
                "url": url,
                "title": doc.get("title", t("tree.no_title"))[:100],
                "freshness_days": freshness_days,
                "freshness_category": freshness_category,
                "freshness_color": freshness_color,
                "last_crawl_days": last_crawl_days,
        ...
        for cache_info in cached_data:
            ...
            if url not in data_map:
                ...
                if filter_site != t("tree.all_sites") and filter_site != site_name:
                    continue
                ...
                if last_crawl_timestamp:
                    ...
                    if crawl_age_days < 1:
                        crawl_text = t("tree.crawled_today")
                    else:
                        crawl_text = t("tree.crawled_days_ago").format(
                            days=crawl_age_days
                        )
                data_map[url] = {
                    "site": site_name or parsed.netloc,
                    "path_parts": path_parts,
                    "page": path_parts[-1][:40] if path_parts else t("tree.homepage"),
                    "url": url,
                    "title": t("tree.pending_indexing"),
                    "freshness_days": 9999,
                    "freshness_category": t("tree.pending_status"),
        ...
        if df.empty:
            ...
            col3.metric(
                t("tree.index_age"),
                f"{avg_freshness:.0f}j"
                if pd.notna(avg_freshness)
                else t("tree.not_available"),
            )
            col4.metric(
                t("tree.indexed_less_than_7_days"), len(df[df["freshness_days"] < 7])
            )
            pending_count = len(df[df["status"] == "pending"])
            if pending_count > 0:
                col5.metric(t("tree.pending_pages"), pending_count)
            else:
                col5.metric(
                    t("tree.crawled_less_than_7_days"),
                    len(df[df["last_crawl_days"] < 7]),
                )

            st.markdown("---")
            col1, col2, col3 = st.columns(3)
            if stale_pages := len(df[df["last_crawl_days"] > 30]):
                col1.metric(
                    t("tree.to_recrawl"), stale_pages, help=t("tree.to_recrawl_help")
                )
            if old_indexed := len(df[df["freshness_days"] > 90]):
                col2.metric(
                    t("tree.old_indexing"),
                    old_indexed,
                    help=t("tree.old_indexing_help"),
                )
            if pending_count > 0:
                col3.metric(t("tree.queue"), pending_count, help=t("tree.queue_help"))

            labels, parents, values, colors, hover_texts, ids = (
                [t("tree.all_pages_root")],
                [""],
                [len(df)],
                [30],
                [t("tree.root_hover").format(total=len(df))],
                ["root"],
            )
            node_map = {"": "root"}
            id_counter = 0
            ...
            for site_name in df["site"].unique():
                ...
                colors.append(30)
                hover_texts.append(
                    t("tree.site_hover").format(site=site_name, count=len(site_data))
                )
                ids.append(site_id)
                ...
                for _, row in site_data.iterrows():
                    ...
                    for part in row["path_parts"][:-1]:
                        ...
                        if current_path not in node_map:
                            id_counter += 1
                            folder_id = f"folder_{id_counter}"
                            labels.append(part[:30])
                            parents.append(current_parent_id)
                            values.append(1)
                            colors.append(30)
                            hover_texts.append(
                                t("tree.folder_hover").format(folder=part)
                            )
                    ...
                    if row["status"] == "pending":
                        colors.append(181)
                        hover_texts.append(
                            t("tree.pending_hover").format(
                                url=row["url"][:60], crawled_text=row["last_crawl_text"]
                            )
                        )
                    else:
                        colors.append(row["freshness_days"])
                        hover_texts.append(
                            t("tree.indexed_hover").format(
                                title=row["title"],
                                category=row["freshness_category"],
                                crawled_text=row["last_crawl_text"],
                                url=row["url"],
                                url_short=row["url"][:50],
                            )
                        )
            ...
            with st.expander(t("tree.how_to_navigate")):
                st.markdown(
                    f"{t('navigation_help_title')}\n{t('navigation_help_1')}\n{t('navigation_help_2')}\n{t('navigation_help_3')}"
                )
                st.markdown(
                    f"{t('color_code_title')}\n{t('color_code_1')}\n{t('color_code_2')}\n{t('color_code_3')}\n{t('color_code_4')}\n{t('color_code_5')}"
                )
            ...
            fig = go.Figure(
                fig_class(
                    ...
                    marker={
                        ...
                        "cmax": 181,
                        "colorbar": {
                            "title": t("tree.colorbar_title"),
                            "tickvals": [0, 7, 30, 90, 180],
                            "ticktext": [
                                t("tree.today"),
                                f"7{t('tree.days_abbr')}",
                                f"30{t('tree.days_abbr')}",
                                f"90{t('tree.days_abbr')}",
                                f"180{t('tree.days_abbr')}+",
                            ],
                        },
                    },
                    text=labels,
                    customdata=hover_texts,
                    hovertemplate=t("tree.hover_size"),
                )
            )
            fig.update_layout(
                height=800,
            ...
            freshness_counts = (
                df["freshness_category"]
                .value_counts()
                .reindex(category_order, fill_value=0)
            )
            freshness_df = pd.DataFrame(
                {
                    t("tree.category"): freshness_counts.index,
                    t("tree.count"): freshness_counts.values,
                }
            )
            fig_freshness = px.bar(
                ...
                x=t("tree.category"),
                y=t("tree.count"),
                title=t("tree.freshness_chart_title"),
                color=t("tree.category"),
            ...
            with col1:
                st.subheader(t("tree.priority_recrawl_title"))
                st.caption(t("tree.priority_recrawl_caption"))
                df["priority_score"] = (
                    df["last_crawl_days"] * 0.7 + df["freshness_days"] * 0.3
                )
                ...
                if not old_pages.empty:
                    ...
                    display_df = old_pages[
                        ["title", "site", "last_crawl_days", "freshness_days"]
                    ].copy()
                    display_df.columns = [
                        t("tree.col_title"),
                        t("tree.col_site"),
                        t("tree.col_crawl_days"),
                        t("tree.col_index_days"),
                    ]
                    st.dataframe(
                        display_df, width="stretch", hide_index=True, height=400
                    )
                else:
                    st.info(t("tree.all_pages_up_to_date"))

            with col2:
                st.subheader(t("tree.recently_crawled_title"))
                st.caption(t("tree.recently_crawled_caption"))
                recent_pages = df.nsmallest(10, "last_crawl_days")

                if not recent_pages.empty:
                    # Sélectionner et renommer les colonnes pour l'affichage
                    display_df_recent = recent_pages[
                        ["title", "site", "last_crawl_days", "freshness_days"]
                    ].copy()
                    display_df_recent.columns = [
                        t("tree.col_title"),
                        t("tree.col_site"),
                        t("tree.col_crawl_days"),
                        t("tree.col_index_days"),
                    ...
                    st.dataframe(
                        display_df_recent, width="stretch", hide_index=True, height=400
                    )
                else:
                    st.info(t("tree.no_recent_crawls"))

            st.markdown("---")
            csv = df.to_csv(index=False).encode("utf-8")
            st.download_button(
                label=t("tree.export_button"),
                data=csv,
                file_name=f"indexation_analysis_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv",
            )
