...
def get_embedding_stats(force_refresh_key=None):
    ...
    try:
        ...
        has_embedding_field = next((f for f in fields if f.get('name') == 'has_embedding' and f.get('type') == 'bool'), None)
        ...
        if has_embedding_field:
            ...
            with_vectors = count_documents(client, INDEX_NAME, filter_by="has_embedding:true")
            without_vectors = count_documents(client, INDEX_NAME, filter_by="has_embedding:false")
        else:
...
if stats:
    ...
    if not config_ok:
        ...
        st.info("Veuillez mettre à jour le schéma de la collection pour inclure un champ de vecteur (par exemple, 'embedding_vec' de type float[]).")
        st.stop()
    ...
    if counts_available:
        ...
        st.info("ℹ️ Le champ `has_embedding` n'existe pas dans cette collection. Les nouvelles collections l'incluent automatiquement lors du crawl.")
